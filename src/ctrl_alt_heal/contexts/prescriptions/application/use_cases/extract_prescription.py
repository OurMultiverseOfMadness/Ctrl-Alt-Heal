...
bedrock_model = BedrockModel(
    ...
    model_id=settings.bedrock_extract_model_id if settings.bedrock_extract_model_id else "apac.amazon.nova-lite-v1:0",
    region_name=settings.aws_region
)
...
@tool(description="Reads a prescription summary and returns a Prescription Python object", inputSchema=INPUT_SCHEMA)
def extract_prescription(summary) -> Prescription | None:
    ...
    try:
        ...
        results = agent.structured_output(Prescription, 
                        [
                            {"text": summary}
                        ])
